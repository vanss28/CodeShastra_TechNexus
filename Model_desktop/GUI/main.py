    ...
    while True:
        if listening:
            ...
            if ("how are you"
                ...
                notepad_path = "C:\\Program Files\\WindowsApps\\Microsoft.WindowsNotepad_11.2401.26.0_x64__8wekyb3d8bbwe\\Notepad\\Notepad.exe"
                os.startfile(notepad_path)

            elif "open github desktop" in query:
                speak("Opening Github Desktop for you sir")
                discord_path = "C:\\Users\\monil\\AppData\Local\\GitHubDesktop\\GitHubDesktop.exe"
                ...
                speak("what date ?")
                set_cal = take_command().lower()
                speak("what is the title of the event sir?")
                set_cal = take_command().lower()
                speak("what time for th event sir?")
                set_cal = take_command().lower()
                speak("Opening calendar and blocking the event")
                ...
                sp.run(['code'], shell=True)

            elif "open postman" in query:
                speak("Opening postman sir")
                postman_path = "C:\\Users\\monil\\AppData\\Local\\Postman\\Postman.exe"
...
""" 
                ...
                speak(
                    "Everyone who are watching this video, Please subscribe my favourite artist The Weekend "
                ...
                speak("The Weekend")
                pyautogui.typewrite("The Weekend", 0.1)
                time.sleep(1)
                speak("press enter")
                pyautogui.press('enter')
                pyautogui.moveTo(971, 314, 1)
                speak("Here you will see the channel")
                pyautogui.moveTo(1688, 314, 1)
                speak("click here to subscribe the channel")
